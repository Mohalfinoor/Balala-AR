...
const ARView = ({ product, onBack }: { product: Product | null; onBack: () => void; key?: string }) => {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 w-full h-screen z-[200] bg-black overflow-hidden flex flex-col select-none touch-none"
    >
        ...
        {!isScanning && product && (
          ...
          >
            ...
            >
              ...
              <div 
                className="absolute -bottom-6 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full bg-teal-600/95 backdrop-blur-md text-[10px] font-bold text-white flex items-center gap-1.5 shadow-lg border border-teal-500/20 select-none pointer-events-none"
              >
      ...
      <div className="relative z-50 flex flex-col h-full pointer-events-none">
        ...
        <div className="mt-auto pb-6 px-6 pointer-events-auto max-w-md mx-auto w-full bg-gradient-to-t from-black/50 via-black/25 to-transparent">
          ...
          {!isScanning && product && (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-teal-650 font-bold mt-0.5">{product.price}</p>
              </div>
          ...
          <div className="bg-black/65 backdrop-blur-xl p-4 rounded-3xl border border-white/10 flex flex-col gap-3">
            ...
            {/* Direct Native AR Launch Button without option-selectors and without redundant choices */}
            {isMobile && !isScanning && (
              <button 
                onClick={handleLaunchNativeAR}
                className="w-full bg-gradient-to-r from-amber-500 via-orange-500 to-teal-600 hover:brightness-110 active:scale-[0.99] transition-all text-white py-3.5 px-4 rounded-2xl text-xs font-black tracking-wider uppercase flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20 cursor-pointer pointer-events-auto border border-white/10"
              >
                <Sparkles size={16} className="text-amber-200 animate-pulse" />
                <span>Mulai AR Realistis di Ruangan</span>
              </button>
            )}
